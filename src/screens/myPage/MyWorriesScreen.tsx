...
export function MyWorriesScreen(props: MyWorriesScreenProps) {
  ...
  const { isHeaderCollapsed, onScroll } = useScrollPeekHeader();
  const contentHeightClassName = isHeaderCollapsed ? 'h-[820px]' : 'h-[752px]';
  const contentClassName = `qling-received-worries-font ${contentHeightClassName} overflow-y-auto rounded-t-[32px] bg-[#fff1d1] px-4 pt-5 transition-[height] duration-[180ms] ease-out [-webkit-overflow-scrolling:touch] motion-reduce:transition-none`;
        ...
        <div className="mx-auto flex h-full w-full max-w-[480px] justify-center overflow-hidden">
          <div className={canvasClassName} style={{ transform: `scale(${canvasScale})` }}>
            ...
            {props.state.status === 'loading' ? (
              <section className={`relative ${contentClassName}`} onScroll={onScroll}>
                <FigmaTabLoading label={props.state.label} />
              </section>
            ) : props.state.status === 'error' ? (
              <section className={contentClassName} onScroll={onScroll}>
