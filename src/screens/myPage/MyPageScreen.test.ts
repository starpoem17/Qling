...
test('my-page root uses a widened unscaled Figma canvas with flow header and body scrolling', () => {
  ...
  assert.match(html, /height:max\(520px, calc\(var\(--qling-tab-viewport-height\) - 100px - var\(--qling-pwa-direct-topbar-shift\)\)/);
...
test('edit interests screen renders fixed two-column chip grid and exact zero-selection validation', () => {
  ...
  assert.match(html, /w-full rounded-\[19px\]/);
  assert.match(html, /h-\[43px\]/);
  assert.match(html, /tracking-normal/);
  assert.match(html, /top:calc\(730px \+ var\(--qling-pwa-topbar-shift, 0px\)\)/);
...
test('privacy policy screen renders document body in the Figma card layout', () => {
  ...
  assert.match(html, /height:max\(320px, calc\(var\(--qling-tab-viewport-height\) - 100px - var\(--qling-pwa-direct-topbar-shift\) - calc\(108px \+ env\(safe-area-inset-bottom,0px\)\)\)\)/);
  assert.doesNotMatch(html, /min\(752px/);
  assert.doesNotMatch(html, /calc\(876px - var\(--qling-tab-viewport-height\)\)/);
