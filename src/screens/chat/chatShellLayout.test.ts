...
test('chat list and chat room use widened unscaled 480px frames', () => {
  ...
  assert.match(chatRoomScreenSource, /Math\.max\(0, fullViewportHeight - visibleHeight - viewportOffsetTop\)/);
...
test('chat shell routes keep scrolling in route-owned content areas', () => {
  ...
  assert.match(chatRoomScreenSource, /style=\{\{ paddingBottom: 'calc\(28px \+ var\(--chat-input-height\) \+ var\(--chat-keyboard-inset\)\)' \}\}/);
...
test('chat room accounts for visual viewport and document background without recovery logic', () => {
  ...
  assert.match(chatRoomScreenSource, /const viewportOffsetTop = visualViewport\?\.offsetTop \?\? 0/);
  assert.match(chatRoomScreenSource, /const keyboardInset = visualViewport\s*\? Math\.max\(0, fullViewportHeight - visibleHeight - viewportOffsetTop\)\s*: 0/);
  assert.match(chatRoomScreenSource, /const isKeyboardHeightVisible = Math\.max\(keyboardInset, viewportHeightShrink\) >= chatRoomKeyboardOpenThreshold/);
