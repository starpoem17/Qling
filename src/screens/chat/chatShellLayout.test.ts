...
test('chat room autoscroll stays inside the message scroller', () => {
  ...
  assert.doesNotMatch(chatRoomScreenSource, /canAutoScrollAfterViewportChangeRef/);
...
test('chat room top bar is fixed in its own unscaled layer while canvas keeps a collapsible spacer', () => {
  ...
  assert.match(chatRoomScreenSource, /className="h-\[calc\(100px\+var\(--qling-pwa-direct-topbar-shift\)\)\] w-full max-w-\[480px\] shrink-0"[\s\S]*<ChatRoomTopBar[\s\S]*opponent=\{opponent\}[\s\S]*answerAdoptionRatePercent=\{answerAdoptionRatePercent\}[\s\S]*onBack=\{onBack\}[\s\S]*onOpenMenu=\{onOpenMenu\}/);
  assert.match(chatRoomScreenSource, /function ChatRoomTopBarSpacer\(\{ isHidden \}: \{ readonly isHidden: boolean \}\)/);
  assert.match(chatRoomScreenSource, /data-chat-room-top-bar-spacer[\s\S]*className="w-full shrink-0 bg-\[#ff8b3d\]"[\s\S]*style=\{\{ height: isHidden \? 0 : chatRoomTopBarHeightCss \}\}/);
  assert.match(chatRoomScreenSource, /data-chat-room-canvas[\s\S]*<ChatRoomTopBarSpacer isHidden=\{isTopBarHiddenForKeyboard\} \/>/);
  assert.match(chatRoomScreenSource, /data-chat-room-top-bar[\s\S]*className="relative z-20 grid h-\[calc\(100px\+var\(--qling-pwa-direct-topbar-shift\)\)\] w-full shrink-0 touch-none grid-cols-\[44px_minmax\(0,1fr\)_44px\] items-start overflow-hidden overscroll-none bg-\[#ff8b3d\] px-\[6px\] pt-\[calc\(45px\+var\(--qling-pwa-direct-topbar-shift\)\)\] qling-figma-font"[\s\S]*onTouchMove=\{blockStaticScroll\}[\s\S]*onWheel=\{blockStaticScroll\}/);
  assert.match(chatRoomScreenSource, /className="flex h-\[45px\] w-\[44px\] items-center justify-center/);
  assert.match(chatRoomScreenSource, /className="flex h-\[45px\] min-w-0 items-center justify-center gap-\[14px\] px-2"/);
  assert.match(chatRoomScreenSource, /className="mr-2 flex h-\[45px\] w-9 items-center justify-center justify-self-end/);
...
test('chat shell routes fill the app shell without extending under bottom navigation', () => {
  assert.match(chatRoomScreenSource, /<section[\s\S]*className="fixed inset-0 z-40 overflow-hidden bg-\[#fff1d1\]"/);
  assert.doesNotMatch(chatRoomScreenSource, /style=\{rootStyle\}/);
...
test('chat list and chat room use widened unscaled 480px frames', () => {
  ...
  assert.match(chatRoomScreenSource, /const \[viewportMetrics, setViewportMetrics\] = useState<ChatRoomViewportMetrics>\(\{ keyboardInset: 0 \}\)/);
  ...
  assert.doesNotMatch(chatRoomScreenSource, /const viewportHeight = visualViewport \? visibleHeight : layoutHeight/);
  assert.match(chatRoomScreenSource, /const viewportOffsetTop = visualViewport\?\.offsetTop \?\? 0/);
  assert.match(chatRoomScreenSource, /const rawKeyboardInset = visualViewport\s*\? Math\.max\(0, fullViewportHeight - visibleHeight - viewportOffsetTop\)\s*: 0/);
  assert.match(chatRoomScreenSource, /const keyboardInset = isChatRoomIosSafariBrowser\(\) \? 0 : rawKeyboardInset/);
  assert.match(chatRoomScreenSource, /keyboardInset/);
  assert.doesNotMatch(chatRoomScreenSource, /canvasHeight: viewportHeight/);
  ...
  assert.match(chatRoomScreenSource, /viewportOffsetTop/);
  assert.doesNotMatch(chatRoomScreenSource, /marginTop/);
  assert.doesNotMatch(chatRoomScreenSource, /const rootStyle/);
  assert.doesNotMatch(chatRoomScreenSource, /translateY\(\$\{viewportMetrics\./);
  ...
  assert.match(chatRoomScreenSource, /const pendingTopBarHideTimeoutRef = useRef<number \| null>\(null\)/);
  assert.match(chatRoomScreenSource, /const chatRoomTopBarHeightCss = `calc\(\$\{chatRoomTopBarHeight\}px \+ var\(--qling-pwa-direct-topbar-shift\)\)`/);
  assert.match(chatRoomScreenSource, /const topBarLayerStyle: CSSProperties = \{\s*height: isTopBarHiddenForKeyboard \? 0 : chatRoomTopBarHeightCss,\s*transform: isTopBarHiddenForKeyboard \? `translateY\(calc\(-1 \* \$\{chatRoomTopBarHeightCss\}\)\)` : undefined,\s*\}/);
  assert.doesNotMatch(chatRoomScreenSource, /paddingTop: `\$\{isTopBarHiddenForKeyboard \? 0 : viewportMetrics/);
  assert.match(chatRoomScreenSource, /\}, \[messages, textareaHeight, viewportMetrics\.keyboardInset, isTopBarHiddenForKeyboard\]\)/);
  assert.match(chatRoomScreenSource, /<section[\s\S]*className="fixed inset-0 z-40 overflow-hidden bg-\[#fff1d1\]"/);
  assert.doesNotMatch(chatRoomScreenSource, /style=\{rootStyle\}/);
  assert.doesNotMatch(chatRoomScreenSource, /transform: `scale\(/);
  assert.doesNotMatch(chatRoomScreenSource, /height: `\$\{viewportMetrics\./);
...
test('chat shell routes keep scrolling in route-owned content areas', () => {
  ...
  assert.match(chatRoomScreenSource, /function ChatRoomInputLayer\(\{ children \}: \{ readonly children: ReactNode \}\)/);
  assert.match(chatRoomScreenSource, /data-chat-room-input-layer[\s\S]*className="fixed left-0 right-0 z-50 mx-auto w-full max-w-\[480px\]"[\s\S]*style=\{\{ bottom: 'var\(--chat-keyboard-inset\)' \}\}/);
...
test('chat room accounts for visual viewport and document background without recovery logic', () => {
  ...
  assert.match(chatRoomScreenSource, /const rawKeyboardInset = visualViewport\s*\? Math\.max\(0, fullViewportHeight - visibleHeight - viewportOffsetTop\)\s*: 0/);
  assert.match(chatRoomScreenSource, /const keyboardInset = isChatRoomIosSafariBrowser\(\) \? 0 : rawKeyboardInset/);
  assert.match(chatRoomScreenSource, /const isKeyboardHeightVisible = Math\.max\(rawKeyboardInset, viewportHeightShrink\) >= chatRoomKeyboardOpenThreshold/);
  assert.match(chatRoomScreenSource, /function isChatRoomIosSafariBrowser\(\)/);
  assert.match(chatRoomScreenSource, /classList\.contains\('qling-ios-safari-browser'\)/);
  assert.match(chatRoomScreenSource, /fullViewportHeightRef\.current = Math\.max\(fullViewportHeight, nextFullViewportHeight\)/);
  assert.match(chatRoomScreenSource, /const clearScheduledTopBarHide = \(\) => \{/);
  assert.match(chatRoomScreenSource, /window\.clearTimeout\(pendingTopBarHideTimeoutRef\.current\)/);
  assert.match(chatRoomScreenSource, /const scheduleSettledTopBarHide = \(\) => \{/);
  assert.match(chatRoomScreenSource, /pendingTopBarHideTimeoutRef\.current = window\.setTimeout\(\(\) => \{/);
  assert.match(chatRoomScreenSource, /\}, chatRoomKeyboardSettleDelayMs\)/);
  assert.match(chatRoomScreenSource, /visualViewport && !isTopBarHiddenForKeyboard && keyboardInputIntentRef\.current && isKeyboardHeightVisible/);
  assert.match(chatRoomScreenSource, /scheduleSettledTopBarHide\(\)/);
  assert.match(chatRoomScreenSource, /visualViewport && isTopBarHiddenForKeyboard && !isKeyboardHeightVisible/);
  assert.match(chatRoomScreenSource, /clearScheduledTopBarHide\(\);\s*keyboardInputIntentRef\.current = false;\s*setIsTopBarHiddenForKeyboard\(false\)/);
  assert.match(chatRoomScreenSource, /visualViewport && !isKeyboardHeightVisible/);
...
test('chat room supports left edge back swipe without global touch listeners', () => {
  ...
  assert.match(chatRoomScreenSource, /const handleMessagesTouchMove = \(event: TouchEvent<HTMLElement>\) =>/);
  assert.match(chatRoomScreenSource, /handleBackSwipeMove\(event\);\s*if \(!isMessageScrollerScrollable\) blockStaticScroll\(event\)/);
  assert.match(chatRoomScreenSource, /deltaX >= chatRoomBackSwipeMinDistance/);
  assert.match(chatRoomScreenSource, /onBack\(\)/);
  assert.match(chatRoomScreenSource, /data-chat-room-message-scroller[\s\S]*onTouchStart=\{handleBackSwipeStart\}/);
  assert.match(chatRoomScreenSource, /data-chat-room-message-scroller[\s\S]*onTouchMove=\{handleMessagesTouchMove\}/);
  assert.match(chatRoomScreenSource, /data-chat-room-message-scroller[\s\S]*onTouchEnd=\{resetBackSwipe\}/);
  assert.doesNotMatch(chatRoomScreenSource, /<section\s+className="fixed inset-0 z-40 overflow-hidden bg-\[#fff1d1\]"\s+onTouchStart=\{handleBackSwipeStart\}/);
...
test('chat room keyboard diagnostics log repeated focus viewport state in dev or PWA opt-in mode', () => {
  ...
  assert.match(chatRoomScreenSource, /if \(pendingTopBarHideTimeoutRef\.current !== null\) \{\s*window\.clearTimeout\(pendingTopBarHideTimeoutRef\.current\);\s*pendingTopBarHideTimeoutRef\.current = null;\s*\}\s*setIsTopBarHiddenForKeyboard\(false\);\s*logChatRoomKeyboardMetric\('textarea\.blur'\)/);
